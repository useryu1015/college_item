...
void Show_Temp(void)
{
	...
	OLED_ShowNum(6,6,DS18B20_Get_Temp()/10,2,16);		
	//OLED_ShowString(24,6,"C",16);
	OLED_ShowCHinese(22,6,29);					// ℃
...
void Show_Home(void)
{
	...
	while(1)
	{
		...
		if ((key = KEY4x4_Scan()) == KEY_ENTER)				
		{
			User_Password();			// 管理员身份认证;
			Show_Menu();				// 进入MENU系统操作界面
		}
			
		Alarm_System(key);				// 警报系统； 	*目前是调用该函数的第一个位置；
		//HW_LED_StateFlag();			// 红外传感器状态指示灯；  // clock.c 中断函数执行；
		Key_Switch();					// 警报开关&照明灯开关；   // 系统中断函数执行；
