...
// 显示汉字，8x8的；																/*********************************** 11/13 *************************/
// 可以尝试修改，自定义大小，但是字库要设置好...
void OLED_ShowCHinese8x8(u8 x,u8 y,u8 no)
{
	u8 t,adder=0;
	OLED_Set_Pos(x,y);	
...
// 使用显示空白的方式，清零													/**** 11/14 由于空格只能清除左半区域（空格的独特属性），出此下册*********/
// x1： 清除多少列(1-16)		LINE?
// y1： 清除多少行(1-8)			ROW?
// x和y则表示从几行几列开始清除；
// 函数描述： 从x、y开始 显示一个大小为x1、y1的矩形空白；
void OLED_Clear8x8(u8 x,u8 y,u8 x1, u8 y1)
{
	int i,j;
	
	if (x1+x/8 > 16) x1 = 16-x/8;						// 11/14;   y1设置为128； OLED屏坏掉，未找出原因？？？？艹  // 溢出保护代码；
	if (y1+y > 8) y1 = 8-y;
	
	
	for (i = 0; i < x1; i++)
		for (j = 0; j < y1; j++)
			OLED_ShowCHinese8x8(x+i*8,y+j,0);	// 显示空白..
