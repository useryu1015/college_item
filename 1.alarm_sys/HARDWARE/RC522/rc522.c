...
void RC522_Handel(void)
{
    ...
    {
        status=MI_ERR;		
        ShowID(0,200,SN,5,4); //在液晶屏上显示卡的ID号			// 原句：ShowID(0,200,SN,BLUE,WHITE);
...
void SPI1_Init(void)	
{
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; 		 //推挽输出   // 浮空输入！
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
    GPIO_Init(GPIOA, &GPIO_InitStructure);
...
void ShowID(u16 x,u16 y, u8 *p, u16 charColor, u16 bkColor)	 //显示卡的卡号，以十六进制显示
{
	...
	u8 i;
 	...
 	for(i=0;i<4;i++)																			// num能正确识别卡的ID号，原理未知；
	{
		num[i*2]=p[i]/16;
...
// 2020/12/1 射频IC卡 扫描函数；   
// 注意：是张卡都能触发该函数？
// 			 测试函数，没有任何安全性；
int RC522_IcCard_Scan(void)
{
	static unsigned char ICstatus = 2;
	//RC522_Handel();				// 寻卡并带识别功能；
	
	ICstatus = PcdRequest(PICC_REQALL,CT);//寻卡
	//printf("ID是%d\r\n", ICstatus);
